### March 18, 2008, use 200 as background cutoff
...
fields = c('chr', 'meanfold.sor55.by.wt', 'medianfold.sor55.by.wt', 'p.paired', 'gene.num','gene.num.above.bgrd');
...
for ( i in 1:length(chrs)) {
	...
	tmp0 = out.backup[out.backup$chr== mychr,] ;
	tmp0 = tmp0[ ! is.na(tmp0[,1]), ]

	tmp = out[out$chr== mychr,] ;
	tmp = tmp[ ! is.na(tmp[,1]), ]

	out.chr$gene.num.above.bgrd[i] = length( tmp$chr );
	out.chr$gene.num[i] = length( tmp0$chr );
...
write.table(out.chr, "_out.sor55.by.chr.031809.csv", sep="\t", row.name=F, quote=F);
