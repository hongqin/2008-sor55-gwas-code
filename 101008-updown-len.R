...
for ( i in c(5,6,8) ) {
	...
	## now combine tables
	#exp.chrtb$start = tb$start[ match( exp.chrtb$orf, tb$orf ) ]; # 100208
	#exp.chrtb$end   = tb$end[ match( exp.chrtb$orf, tb$orf ) ]; #  100208	
	#exp.chrtb$strand= tb$strand[ match( exp.chrtb$orf, tb$orf ) ]; #  100208	
	#exp.chrtb$loc = exp.chrtb$start /2  + exp.chrtb$end/2 
	#summary(exp.chrtb) #25 NAs ??, maybe, 
	#exp.chrtb[is.na(exp.chrtb$start),]
	#head(exp.chrtb);
	#tb[grep("orf19.4163",tb$orf),] #
	...
	tt = table( sliding.test$clustertype)
