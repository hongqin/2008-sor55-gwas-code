### 031809 use 200 as cutoff, add MTL back
...
expecteds = c(0.05, 0.9, 0.05) * window;
 ...
 pdf( "_031809-chr5-chisq.pdf",height=3,width=4);
...
exp.chrtb = exp.chrtb[ ! is.na(exp.chrtb$chr), ]
str( exp.chrtb );
...
#	} 
	...
	for( j in 1:length(exp.chrtb$orf) ) {
		...
		} else if (  yin < 0.6  ) { mycol = 'red'; ##020909 bulgac change
		} else { mycol= 'green'; } 
