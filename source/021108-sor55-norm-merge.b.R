...
w2.all$FGMean2 = w2.all$FGMean2 * scale / sum(w2.all$FGMean2[w2.all$geneflag==1], na.rm=T)
w3.all$FGMean2 = w3.all$FGMean2 * scale / sum(w3.all$FGMean2[w3.all$geneflag==1], na.rm=T)
w4.all$FGMean2 = w4.all$FGMean2 * scale / sum(w4.all$FGMean2[w4.all$geneflag==1], na.rm=T)

m1.all$FGMean2 = m1.all$FGMean2 * scale / sum(m1.all$FGMean2[m1.all$geneflag==1], na.rm=T)
m2.all$FGMean2 = m2.all$FGMean2 * scale / sum(m2.all$FGMean2[m2.all$geneflag==1], na.rm=T)
m3.all$FGMean2 = m3.all$FGMean2 * scale / sum(m3.all$FGMean2[m3.all$geneflag==1], na.rm=T)
m4.all$FGMean2 = m4.all$FGMean2 * scale / sum(m4.all$FGMean2[m4.all$geneflag==1], na.rm=T)
...
#write.table(out, "_out.sor55.021108.csv", col.names=T, row.names=F, quote=F, sep="\t");

###
 
 up = out[ (out$p.paired<0.01) & (out$fold.mut.by.wt > 1.5), ]
 up = up[ up$geneflag==1,]
 #write.table(up, "_out.Sor55.up.021108.csv", col.names=T, row.names=F, quote=F, sep="\t");

 down = out[ (out$p.paired<0.01) & (out$fold.mut.by.wt < 0.5), ]
 #write.table(down, "_out.Sor55.down.021108.csv", col.names=T, row.names=F, quote=F, sep="\t");
...
#write.table(bottom, "_out.Sor55.bottom5percent.021108.csv", col.names=T, row.names=F, quote=F, sep="\t");

#save.image("021108.bkgrd.corrected.RData")
