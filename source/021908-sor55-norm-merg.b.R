...
###
 ...
 up = up[ up$geneflag==1,]
 #write.table(up, "_out.sor55.up.021908.csv", col.names=T, row.names=F, quote=F, sep="\t");

 down = out[ (out$p.paired<0.01) & (out$fold.mut.by.wt < 0.5), ]
 #write.table(down, "_out.sor55.down.021908.csv", col.names=T, row.names=F, quote=F, sep="\t");
...
#write.table(bottom, "_out.sor55.bottom5percent.021908.csv", col.names=T, row.names=F, quote=F, sep="\t");

#save.image( "021908.bkgrdCorrected.nonChr5Normalized.RData" );
