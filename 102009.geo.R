...
w2.all$FGMean2 = w2.all$FGMean2 * scale / sum( w2.all$FGMean2[(w2.all$chr != 'Chr5') &   (w2.all$geneflag==1)],na.rm=T);

w3.all$FGMean2 = w3.all$FGMean2 * scale / sum( w3.all$FGMean2[(w3.all$chr != 'Chr5') & (w3.all$geneflag==1)],na.rm=T);

#w4.all$FGMean3 = w4.all$FGMean * scale / sum( w4.all$FGMean2[(w4.all$chr != 'Chr5') & (w4.all$geneflag==1)],na.rm=T);

m1.all$FGMean2 = m1.all$FGMean2 * scale / sum( m1.all$FGMean2[(m1.all$chr != 'Chr5') & (m1.all$geneflag==1)],na.rm=T);

m2.all$FGMean2 = m2.all$FGMean2 * scale / sum( m2.all$FGMean2[(m2.all$chr != 'Chr5') & (m2.all$geneflag==1)],na.rm=T);

m3.all$FGMean2 = m3.all$FGMean2 * scale / sum( m3.all$FGMean2[(m3.all$chr != 'Chr5') & (m3.all$geneflag==1)],na.rm=T);
...
c(check1, check2, checkm1, checkm2, checkm3); #these should be 1, 1, 1, 1, 1

w1.all$ID_REF = 1:length(w1.all[,1])
w2.all$ID_REF = 1:length(w2.all[,1])
w3.all$ID_REF = 1:length(w3.all[,1])
m1.all$ID_REF = 1:length(m1.all[,1])
m2.all$ID_REF = 1:length(m2.all[,1])
m3.all$ID_REF = 1:length(m3.all[,1])

platform1 = data.frame ( w1.all$ID_REF );
names(platform1) = c("ID_REF");
#platform1$name = as.character( w1.all$Name );
platform1$orf = as.character( w1.all$orf );
platform1$Sequence = as.character( w1.all$Sequence );
platform1$w1 = w1.all$FGMean2 
platform1$w2 = w2.all$FGMean2 
platform1$m1 = m1.all$FGMean2 
platform1$m2 = m2.all$FGMean2 
platform1$raw_w1 = w1.all$FGMean;
platform1$raw_w2 = w2.all$FGMean;
platform1$raw_m1 = m1.all$FGMean;
platform1$raw_m2 = m2.all$FGMean;

platform2 = data.frame ( w3.all$ID_REF );
names(platform2) = c("ID_REF");
#platform1$name = as.character( w1.all$Name );
platform2$orf = as.character( w3.all$orf );
platform2$Sequence = as.character( w3.all$Sequence );
#platform2 = data.frame ( cbind( as.character(w3.all$orf) ) );
#platform2$ID_REF = w3.all$ID_REF 
platform2$w3 = w3.all$FGMean2
platform2$m3 = m3.all$FGMean2 
platform2$raw_w3 = w3.all$FGMean 
platform2$raw_m3 = m3.all$FGMean 

write.table(platform1, "_platform1.geo.102009.csv", col.names=T, row.names=F, quote=F, sep="\t");
write.table(platform2, "_platform2.geo.102009.csv", col.names=T, row.names=F, quote=F, sep="\t");
...
for( myorf in as.character( orfs ) ) {
 ...
 write.table(data, "_probe.data.merged.sor55.nonCh5norm.092909.csv", col.names=T, row.names=F, quote=F, sep="\t");
...
write.table(out, "_out.sor55.nonCh5norm.092909.csv", col.names=T, row.names=F, quote=F, sep="\t");
...
save.image( "092909.myannotation.bkgrdCorrected.nonChr5Normalized.RData" );
