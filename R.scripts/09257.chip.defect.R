...
### plot images
pdf ( paste("_image.sorbose.pdf"), width=10 ) 

for( i in 1:3 ) {

w1.all = read.delim( wildtype.files[i], header=T, sep="\t", fill=T); 
...
image( w1.mat );
title ( wildtype.files[i]  )
}


for( i in 1:3 ) {

w1.all = read.delim( mutant.files[i], header=T, sep="\t", fill=T); 
max(w1.all$Row, na.rm=T); #224 rows
max(w1.all$Column, na.rm=T); #56 columns
unique(w1.all$orf);  #6372 levels
w1.mat = matrix( nrow=224, ncol=56);
...
w1.mat = ifelse( w1.mat>ceilings[i+3], ceilings[i+3], w1.mat);
image( w1.mat );
title ( mutant.files[i] )
