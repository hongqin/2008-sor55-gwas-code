...
### histogram of expression ratio along chromosomes 
### modified from 013008-plot-chr-chisq.R
...
out$WtMean  = out$WtMean  - wt.background;
out$MutMean = out$MutMean - mut.background;

out$WtMean[out$WtMean<0] = 1;
out$MutMean[out$MutMean<0] = 1;

out$fold.mut.by.wt = out$MutMean / out$WtMean; 
...
window.bp = 0.5E5;  window = 25;  pcutoff = 1E-4;  #this seems nice
#window.bp = 0.5E5;  window = 15;  pcutoff = 1E-6;
#window.bp = 0.5E5;  window = 15;  pcutoff = 1E-6;

expecteds = c(0.05, 0.9, 0.05) * window;
		
pdf( "_020508-chrs-chisq.pdf",height=10,width=8);
	...
	for( j in 1:(length(sliding.test$loc) - window + 1) ) {
		...
		if ( sliding.test$pchisq[ center ] < pcutoff ) {
		  if ((obs[1]> expecteds[1]) && (obs[3]< expecteds[3]) ){
		   ...
		   MyBox( sliding.test$loc[center],yplot,yscale/100,xscale/200,
...
###now the histogram
pdf( "_020508-chrs-hist.pdf",height=10,width=8);
...
write.table(out.chr, "_out.by.chr.020508.after.background.correction.csv");
