...
for ( i in 1:length(chrs)) {
	...
	#for t-test 
        tmp$WtMean2   = ifelse( tmp$WtMean2<=0,  1, tmp$WtMean2);
        tmp$MuttMean2 = ifelse( tmp$MutMean2<=0, 1, tmp$MutMean2);
...
write.table(out.chr, "_out.sor55.by.chr.022108.csv", sep="\t", row.name=F, quote=F);
