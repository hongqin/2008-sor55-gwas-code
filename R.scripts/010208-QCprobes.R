...
probe.sums = c( sum(pw1.all$FGMean), 
	sum(pw2.all$FGMean),
	sum(pw3.all$FGMean),
	sum(pm1.all$FGMean), 
	sum(pm2.all$FGMean),
...
all.sums = c( sum(w1.all$FGMean, na.rm=T), 
	sum(w2.all$FGMean, na.rm=T),
	sum(w3.all$FGMean, na.rm=T),
	sum(m1.all$FGMean, na.rm=T), 
	sum(m2.all$FGMean, na.rm=T),
	sum(m3.all$FGMean, na.rm=T)
)

all.sums / probe.sums; #this show that "all sums" normalization is OK

scale = sum( w1.all$FGMean[w1.all$geneflag==0], na.rm=T);
...
### found out all probes
QCs = unique( pm1.all$orf );

out = data.frame( matrix(nrow=length( QCs ), ncol=7 ) );
names(out) = c( "orf", "chr", "gene", "WtMean","MutMean", "fold.mut.by.wt", "p");

 i = 0;
for( myorf in as.character( QCs ) ) {
 i = i + 1;
 wt = c(); mut = c();
...
summary(w1.all$FGMean);
 ...
 write.table(out, "_out.Sor55.normalizedbyQC.010208.csv", col.names=T, row.names=F, quote=F, sep="\t");

###
 up = out[ (out$p <0.01) & (out$fold.mut.by.wt > 1.5), ]
 write.table(up, "_out.Sor55.up.0102008.csv", col.names=T, row.names=F, quote=F, sep="\t");

 down = out[ (out$p <0.01) & (out$fold.mut.by.wt < 0.5), ]
 write.table(down, "_out.Sor55.down.010208.csv", col.names=T, row.names=F, quote=F, sep="\t");
