## Oct13,08 check 2fold down genes, gearmoran{ade4}
...
#for ( i in c(5,6,8) ) {
	...
	Wtb = exp.chrtb[exp.chrtb$strand=='W',]
	Ctb = exp.chrtb[exp.chrtb$strand=='C',]
	...
	red.adj = matrix( nrow=length(red.tb[,1]), ncol=length(red.tb[,1]) );
	num.cutoff = 1; #only adjacent orfs are neighbours
	
	for( i in 1:length(red.adj[,1] ) ) {  #row
	   for( j in 1:length(red.adj[1,] ) ) { #column
	      
	   }
	}
	
	# generate adjacency matrix for "red"s.
